(() => {
  ...
  const GLOSSARY = {
    ...
    "Funding Rate": "Комиссия между лонгами и шортами на фьючерсах. Отрицательный = шорты платят лонгам (бычий знак). Высокий положительный = рынок перегрет.",
    "Open Interest": "Сумма всех открытых позиций на бирже. Растёт = интерес к рынку. Падает = трейдеры выходят.",
    "Тренд": "Направление движения цены. Восходящий = цена растёт. Нисходящий = падает. Боковой = ходит в диапазоне.",
    "Монета": "Криптовалюта, которую анализирует бот.",
    "Цена": "Текущая рыночная цена монеты.",
  };
